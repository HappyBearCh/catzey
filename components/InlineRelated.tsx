...
export function InlineRelated({ articles, basePath = '' }: Props) {
  ...
  return (
    <div className="my-8 not-prose">
      <p className="text-2xs font-black uppercase tracking-widest text-primary mb-3">
        {isBosnian ? 'Više priča' : 'More Stories'}
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-site-light dark:bg-gray-800 border-l-4 border-primary">
        {articles.slice(0, 2).map((article) => {
          const label = isBosnian ? getCategoryLabelBs(article.category) : getCategoryLabel(article.category);
