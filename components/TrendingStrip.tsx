...
export async function TrendingStrip({ basePath = '' }: TrendingStripProps) {
  ...
  let articles = await prisma.article.findMany({
    ...
    orderBy: { views: 'desc' },
    take: 5,
    select: { id: true, slug: true, title: true, titleBs: true, category: true, views: true },
  });

  // Fall back to most-viewed overall if not enough recent trending
      ...
      <div className="max-w-8xl mx-auto px-4 py-2 flex items-center gap-4 overflow-x-auto scrollbar-hide">
        ...
        <div className="flex items-center gap-0 divide-x divide-site-border">
          {articles.map((article) => {
            ...
            const catLabel = isBosnian ? getCategoryLabelBs(article.category) : getCategoryLabel(article.category);
            return (
              <Link
                key={article.id}
