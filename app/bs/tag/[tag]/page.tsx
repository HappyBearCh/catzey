...
import { bsArticle } from '@/lib/types';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `#${label}`,
    description: `Članci označeni sa "${label}" na Catzye.`,
  };
}

export default async function BosnianTagPage({ params }: Props) {
  const { tag } = await params;
  const label = decodeTag(tag);
  ...
  return (
    <div className="max-w-8xl mx-auto px-4 py-6">
      ...
      <p className="text-site-gray text-sm mb-8 ml-4">
        {articles.length} {articles.length === 1 ? 'članak' : 'članaka'} označenih sa &quot;{label}&quot;
      </p>
